...
def retrieve(mode, word, *args): #these arguments require a mode and atleast one query word but will accept any number of them
    ...
    if mode.lower() == "or" or mode.lower() == "most" or mode.lower() == "and":
        ...
        if len(queryWords) == 2:
            phrase = queryWords[1] + " " +queryWords[0]
        ...
        if len(hitSet) > 0:
            ...
            if len(scoreList) > 25: 
                for i in range(25): #prints the first 25 hits in order of tf-idf if there are more than 25 hits
                    print("Url:", scoreList[i][1])
                    print("Title:", str(scoreList[i][2]))
                    print("Total Rank:", scoreList[i][0])
                    print()
                print("Number of pages searched:", numberOfFiles)
                print("Number of hits:", len(hitSet))
                return None
            else:
                for page in scoreList: #prints each page in the tfidf list
                    print("Url:", page[1])
                    print("Title:", str(page[2]))
                    print("Total Rank:", page[0])
                    print()
                print("Number of pages searched:", numberOfFiles)
                print("Number of hits:", len(hitSet))
                return None
...
def main(*args):
    ...
    parser.add_argument('mode' , help='The mode you would like to use to retrieve the pages. Options are "OR", "AND", or "MOST. OR means to return pages that include any of the keywords; AND means to return pages that include all of the keywords; MOST means to return pages that include most (at least half) of the keywords.', type = str)
    ...
    retrieve(args.mode, args.word, *args.words)
